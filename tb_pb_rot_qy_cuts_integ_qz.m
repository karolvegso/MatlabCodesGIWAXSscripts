...
% then takes qy-cuts (horizontal cuts along qy axis) 
% integrated in qz interval
%**************************************************************************
% path to folder with images
path_to_input_folder='d:\Pilatus_images\';
% path to folder with result qy cuts matrix
...
start_pixel_row=1; % [pixel]
% stop pixel in row direction
stop_pixel_row=407; % [pixel]
...
start_pixel_col=1; % [pixel]
% stop pixel in col direction
stop_pixel_col=487; % [pixel]
...
%**************************************************************************
% linecut parameters - horizontal cut along qy axis
%**************************************************************************
% number of qy points to cut
no_of_qy_points=300;
% qy - minimum limit
qy_min=-0.33; % [A^(-1)]
% qy - maximum limit
...
for index_0=no_image_start:time_binning:no_image_stop
    ...
    for index_1=index_0:(index_0+time_binning-1)
        ...
        %******************************************************************
        % create gixsdata object for existing image
        obj=gixsdata(full_path_to_image);
        % calibarte existing image
    ...
    % define constraint
    constr = [1 3 qz_min qz_max; 1 5 qy_min qy_max]; 
    % perform linecut
    xflag = 5; % qy linecut
    nofpts = no_of_qy_points; % number of points in the linecut
...
% save qy-cuts (horizontal cuts along qy axis) matrix
